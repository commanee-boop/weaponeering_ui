...
const coordinateText = targetInfo => {
  if (hasReportValue(targetInfo.coordinates)) return String(targetInfo.coordinates)
...
const summaryValues = data => {
  ...
  return [
    ...
    hasReportValue(targetInfo.heightMslFt) ? targetInfo.heightMslFt : (hasReportValue(targetInfo.heightMsl) ? targetInfo.heightMsl : '-'),
    targetInfo.details,
    targetInfo.desiredResult || targetInfo.desiredEffect || targetInfo.area || firstRecommendation.size,
    targetInfo.weaponUsed || '-'
...
const additionalRows = data => {
  ...
  const rows = [
    ['ประเภทเป้าหมาย', isSameValue(targetInfo.type, summary[5]) ? '' : targetInfo.type],
    ['ชนิดสิ่งก่อสร้าง', isSameValue(targetInfo.structure, summary[5]) ? '' : targetInfo.structure],
    ['พื้นที่/แหล่งข้อมูล', isSameValue(targetInfo.area, desiredResult) ? '' : targetInfo.area],
    ['ความสำคัญเป้าหมาย', isSameValue(targetInfo.targetImportance, summary[2]) ? '' : targetInfo.targetImportance],
    ...
    ['Pk Value', metrics.pk],
    ['CEP Value', hasReportValue(metrics.cep) ? withoutMeterUnit(metrics.cep) : '']
  ]
...
export const exportService = {
  ...
  async exportToWord(data, filename = 'weaponeering_analysis.docx') {
    try {
      ...
      const imageFrameBorders = {
        top: { style: BorderStyle.SINGLE, size: 2, color: wordBorder },
        bottom: { style: BorderStyle.SINGLE, size: 2, color: wordBorder },
        left: { style: BorderStyle.SINGLE, size: 2, color: wordBorder },
        right: { style: BorderStyle.SINGLE, size: 2, color: wordBorder },
        insideHorizontal: { style: BorderStyle.NONE, size: 0, color: wordBorder },
        insideVertical: { style: BorderStyle.NONE, size: 0, color: wordBorder }
      }
